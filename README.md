...
- **改稿后自查**：按 [改稿残句自查表](references/revision-leftovers.md) 过一遍改动处前后各一段（八类残句），再 `duplicates.py` 全书比对一遍（重点看逐字重复的段落与句子，框架句与母题按提示保留），再 `timeline_audit.py` 把作息与时刻排出来，逐条核对「这个点该谁在场」。

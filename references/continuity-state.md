...
一个未公开条目可以在第 0 章状态中写成 `{"truth": "账本最后一页在仓库", "known_by": [], "reader_known": false}`。角色获知时更新 `known_by`；读者在正文中明确获知时同章设 `reader_known: true` 和 `revealed_chapter`。**`known_by` 只记「谁知道了」是不够的，要在条目的 `note` 里写清粒度**：他知道的是哪一层、故意漏掉了什么、对方是否追问过。11 章项目里就吃過这个亏——状态只记「苏莉成为第二个知情人」，而正文里他在坦白时刻意漏掉了袋子有几双、留下了哪几双、是谁扔的，于是同一章后面「还有两双，在北京」被写成了她早该知道的信息，矛盾一路躲过单章审查与状态校验。粒度写进 `note` 后，改稿的人和审稿的人都能看见边界在哪里。章节事务不能静默更改已确定的 `truth`、撤销读者已知或删除条目；如重写旧章改变真相，先审阅后续正文与事务，再按重建流程处理。

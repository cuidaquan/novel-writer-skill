...
3. 先运行 `scripts/project_check.py <project> --preflight book`；通过后对每章依次执行：生成上下文 → 写正文 → 检查因果、POV、连续性和语言 → 写事务 → 提交状态 → 运行 `scripts/project_check.py <project>`。下章只在本章正文与状态稳定后开始。上下文快照是临时输入，不是正文，也不保证后来仍反映最新状态。

...
改完全书之后除了脚本复查，再做一次**残句自查**：按 [改稿残句自查表](revision-leftovers.md) 分 3—4 章一组派人细读，重点不是文风，而是「这一段是不是把上一段又演了一遍」。
...
报告头带正文指纹 `sha256:<12 位>`。`NOTE` 与文风漂移只算一次，正文改了报告不会自动更新；改完正文重跑一次，指纹变了就说明手里那份报告对应的是旧稿，不要拿它当当前定稿依据。提交时的门禁会重跑确定性问题，因此指纹只影响人工与 Agent 引用的判断，不影响能否写入。
